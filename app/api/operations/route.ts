...
const requestSchema=z.object({version:z.number().int().nonnegative(),role:z.enum(['Admin','Operations Manager','Site Lead','Senior Manager','Employee']),action:z.discriminatedUnion('type',[
 z.object({type:z.literal('save-deployment'),site,week:date,placements:z.array(duty).max(150)}),
 z.object({type:z.literal('publish-deployment'),site,week:date}),
 z.object({type:z.literal('discard-deployment'),site,week:date}),
...
export async function POST(req:Request){try{
 ...
 const permissions:Record<Role,string[]>={Admin:['employee','assign','attendance','incident','resolve','leave','save-deployment','publish-deployment','discard-deployment'],'Operations Manager':['assign','attendance','incident','resolve','leave','save-deployment','publish-deployment','discard-deployment'],'Senior Manager':['assign','attendance','incident','resolve','leave','save-deployment','publish-deployment','discard-deployment'],'Site Lead':['attendance','incident','resolve'],Employee:[]};
 ...
 if(action.type==='save-deployment'||action.type==='publish-deployment'||action.type==='discard-deployment'){
  if(weekStart(action.week)!==action.week)return response({error:'Choose a Monday as the week start.'},400);
  try{
   if(action.type==='save-deployment'){savePlan(state,action.site,action.week,action.placements,role,now);summary=`Deployment draft saved · ${action.site} · week of ${action.week}`;}
   if(action.type==='discard-deployment'){state.deploymentDrafts=(state.deploymentDrafts||[]).filter(d=>d.site!==action.site||d.week!==action.week);summary=`Deployment draft discarded · ${action.site}`;}
   if(action.type==='publish-deployment'){const result=publishPlan(state,action.site,action.week,role,now,()=>crypto.randomUUID());summary=`Deployment published · ${action.site} · ${action.week} · ${result.changes.added} added, ${result.changes.removed} replaced/removed · ${result.gaps} gaps remain`;}
  }catch(e){return response({error:(e as Error).message},409)}
 }
 ...
 if(action.type!=='save-deployment')state.audit.unshift({id:crypto.randomUUID(),action:summary,time:now,role});
